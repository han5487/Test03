...
public class Other01 {
	...
	public static void main(String[] args) {
		...
		createPdf("D:\\test\\新增資料夾 (2)\\本票.pdf");
	...
	public static void createPdf(String dest) {
		...
		try {
			...
			List list1 = new List();
			list1 = new List().setFont(baseFont);// 有序列表
			list1.add(new ListItem("本票據利息依下列第  項約定利率計付。").setListSymbol("一、"));
			...
			list2.add("利息自發票日起依年息 ％固定計息，按月計付。");
			list2.add("");
			...
			list1.add(new ListItem("前條約定之利息，如蒙　貴行同意暫按較低利率優惠計息者，貴行並得視情形減少或中止優惠，恢復按原約定利率計息。").setListSymbol("二、"));
			cell = new Cell(1, 50).add(list1);
			table.addCell(cell);
			...
			list1 = new List().setListSymbol(ListNumberingType.DECIMAL).setFont(baseFont);// 有序列表
			list1.add(new ListItem("第三條違約金之計算基準：").setListSymbol("四、"));
			...
			list1 = new List().setListSymbol(ListNumberingType.DECIMAL).setFont(baseFont);// 有序列表
			list1.add(new ListItem("此票免除作成拒絕證書。").setListSymbol("五、"));
			cell = new Cell(1, 50).add(list1);
			table.addCell(cell);

			list1 = new List().setListSymbol(ListNumberingType.DECIMAL).setFont(baseFont);// 有序列表
			list1.add(new ListItem("付款地：").setListSymbol("六、"));
			cell = new Cell(1, 50).add(list1);
			table.addCell(cell);
